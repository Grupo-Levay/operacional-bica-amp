...
export function FichaCard({ ficha }: { ficha: FichaTecnica }) {
  return (
    <Link href={`/fichas/${ficha.id}`} className="block">
      <Card size="sm" className="rounded-lg border shadow-sm transition-shadow hover:shadow-md">
        ...
        <CardContent className="space-y-1 pt-1">
          ...
          {ficha.custo_total != null && (
            <p className="text-xs text-muted-foreground">
              Custo:{' '}
              <span className="font-medium text-foreground">{brl.format(ficha.custo_total)}</span>
            </p>
          )}
          {ficha.preco_venda != null && (
            <p className="text-xs text-muted-foreground">
              Venda:{' '}
              <span className="font-medium text-foreground">{brl.format(ficha.preco_venda)}</span>
            </p>
          )}
