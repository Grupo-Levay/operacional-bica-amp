import { CalendarCheck, ArrowLeft } from 'lucide-react'
import Link from 'next/link'
import { getCurrentCasa } from '@/lib/tenant'
import { getTodayISO } from '@/lib/utils'
import { ReservaCard, type Reserva } from '@/components/reservas/reserva-card'
import { NovaReservaForm } from '@/components/reservas/nova-reserva-form'

async function getReservas(data: string): Promise<Reserva[]> {
  try {
    const { createClient } = await import('@/lib/supabase/server')
    const supabase = await createClient()
    const { data: rows } = await supabase
      .from('reservations')
      .select(
        'id, customer_name, customer_phone, reservation_date, start_time, end_time, guest_count, status, notes, created_by_name',
      )
      .eq('reservation_date', data)
      .order('start_time')
    return (rows ?? []) as Reserva[]
  } catch {
    return []
  }
}

function addDays(iso: string, days: number): string {
  const [y, m, d] = iso.split('-').map(Number)
  const dt = new Date(Date.UTC(y, m - 1, d + days))
  return dt.toISOString().split('T')[0]
}

function fmtDateLabel(iso: string): string {
  const [y, m, d] = iso.split('-').map(Number)
  return new Date(Date.UTC(y, m - 1, d)).toLocaleDateString('pt-BR', {
    weekday: 'long',
    day: 'numeric',
    month: 'long',
    timeZone: 'UTC',
  })
}

type Props = { searchParams: Promise<{ data?: string }> }

export default async function ReservasPage({ searchParams }: Props) {
  const [casa, { data: dataParam }] = await Promise.all([getCurrentCasa(), searchParams])

  const hoje = getTodayISO()
  const dataAtiva = dataParam ?? hoje
  const casaColor = casa === 'bica' ? 'var(--color-bica)' : 'var(--color-amp)'

  const anteriorStr = addDays(dataAtiva, -1)
  const proximoStr = addDays(dataAtiva, 1)
  const isHoje = dataAtiva === hoje

  const reservas = await getReservas(dataAtiva)

  const pendentes = reservas.filter((r) => r.status === 'pendente').length
  const confirmadas = reservas.filter((r) => r.status === 'confirmada').length
  const ativas = reservas.filter((r) => r.status !== 'cancelada' && r.status !== 'concluida')
  const encerradas = reservas.filter((r) => r.status === 'cancelada' || r.status === 'concluida')

  return (
    <main className="p-4 space-y-5 pb-24">
      {/* Header */}
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-3">
          <Link
            href="/dashboard"
            className="flex items-center justify-center size-9 rounded-full border transition-opacity hover:opacity-70"
            style={{ borderColor: 'var(--border)', color: 'var(--color-b3)' }}
            aria-label="Voltar"
          >
            <ArrowLeft size={18} />
          </Link>
          <div>
            <h1 className="text-xl font-bold" style={{ color: casaColor }}>
              Reservas
            </h1>
            <p className="text-xs capitalize" style={{ color: 'var(--color-b4)' }}>
              {fmtDateLabel(dataAtiva)}
            </p>
          </div>
        </div>

        {/* Navegação de data */}
        <div className="flex items-center gap-1">
          <a
            href={`/reservas?data=${anteriorStr}`}
            className="flex items-center justify-center size-8 rounded-full border text-lg leading-none hover:opacity-70 transition-opacity"
            style={{ borderColor: 'var(--border)', color: 'var(--color-b3)' }}
            aria-label="Dia anterior"
          >
            ‹
          </a>
          {!isHoje && (
            <a
              href="/reservas"
              className="text-xs px-2 py-1 rounded border hover:opacity-70 transition-opacity"
              style={{ borderColor: 'var(--border)', color: 'var(--color-b4)' }}
            >
              Hoje
            </a>
          )}
          <a
            href={`/reservas?data=${proximoStr}`}
            className="flex items-center justify-center size-8 rounded-full border text-lg leading-none hover:opacity-70 transition-opacity"
            style={{ borderColor: 'var(--border)', color: 'var(--color-b3)' }}
            aria-label="Próximo dia"
          >
            ›
          </a>
        </div>
      </div>

      {/* Stats */}
      {reservas.length > 0 && (
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: 'Total', value: reservas.length, color: 'var(--color-b1)' },
            { label: 'Pendentes', value: pendentes, color: '#d97706' },
            { label: 'Confirmadas', value: confirmadas, color: '#16a34a' },
          ].map(({ label, value, color }) => (
            <div
              key={label}
              className="rounded-lg border shadow-sm p-3 flex flex-col gap-0.5"
              style={{ backgroundColor: 'var(--color-ink2)', borderColor: 'var(--border)' }}
            >
              <span
                className="text-[10px] font-medium uppercase tracking-wide"
                style={{ color: 'var(--color-b4)' }}
              >
                {label}
              </span>
              <span className="text-2xl font-bold tabular-nums leading-none" style={{ color }}>
                {value}
              </span>
            </div>
          ))}
        </div>
      )}

      {/* Nova reserva */}
      <NovaReservaForm casaColor={casaColor} />

      {/* Lista */}
      <section className="space-y-3">
        {ativas.length === 0 && encerradas.length === 0 ? (
          <div
            className="flex flex-col items-center justify-center gap-3 py-12"
            style={{ color: 'var(--color-b4)' }}
          >
            <CalendarCheck className="size-10 opacity-30" />
            <p className="text-sm">Nenhuma reserva para este dia</p>
          </div>
        ) : (
          <>
            {ativas.length > 0 && (
              <div className="space-y-3">
                {ativas.map((r) => (
                  <ReservaCard key={r.id} reserva={r} casaColor={casaColor} />
                ))}
              </div>
            )}

            {encerradas.length > 0 && (
              <div className="space-y-2">
                <p
                  className="text-xs font-medium uppercase tracking-wide"
                  style={{ color: 'var(--color-b4)' }}
                >
                  Encerradas
                </p>
                {encerradas.map((r) => (
                  <ReservaCard key={r.id} reserva={r} casaColor={casaColor} />
                ))}
              </div>
            )}
          </>
        )}
      </section>
    </main>
  )
}
